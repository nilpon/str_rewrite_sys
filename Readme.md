...
This tab shows all equalities produced during the `Knuth-Bendix` procedure,
which attempts to produce confluent rewriting system.
To use this tab, push the `Knuth-Bendix` button first.
...
The equalities generated during the Knuth-Bendix procedure contains many redundant ones.
Checking `簡約された関係式だけ表示` (show only reduced equalities) hide the redundant
equalities.

You can confirm the confluence of generated equalities by the
`合流的か?`(is confluent) button.

`新しいMonoidにコピー` (copy to new Monoid) button generates a new monoid
having the (reduced) equalities generated by the Knuth-Bendix procedure.
The verification of confluence on this page is VERY SLOW because it does not use
optimized algorithm, which requires some index data.
Thus it is recommended to verify the confluence AFTER COPYING equalities and
use `合流的か?`(is confluent) button on `定義関係式`(Defining relations) tab,
which will use optimized algorithm.
